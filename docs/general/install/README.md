...
gh -R 'pbs/terraform-ecs-service-module' release download -p 'release.tar.gz' x.y.z
tar -xvf release.tar.gz -C 'terraform/modules/ecs-service'
rm -f release.tar.gz
```
...
```bash
git remote add -f ecs-service git@github.com:pbs/terraform-ecs-service-module.git
git subtree add --prefix terraform/modules/ecs-service ecs-service main --squash
```

If necessary, pin the repo version, rather than adding `main` as your subtree:

```bash
git subtree add --prefix terraform/modules/ecs-service ecs-service $REF --squash
```
